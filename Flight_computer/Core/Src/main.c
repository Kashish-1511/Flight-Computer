...
void LoggingTask(void *params)
{
	SensorSample_t sample;
	 ...
	 uint16_t index = 0;
	 uint32_t nextBlock = 100;

    for (;;) {
        if (xQueueReceive(sensorQueue, &sample, portMAX_DELAY)) {
        	if (index + sizeof(SensorSample_t) > 512) {
        	                // Write full block
        	                SD_WriteBlock(nextBlock, sdBuffer);
        	                nextBlock++;  // 🟢 Increment block number for next write
        	                index = 0;
        	                memset(sdBuffer, 0, 512);
        	}
        	 memcpy(&sdBuffer[index], &sample, sizeof(SensorSample_t));
...
void SensorTask(void *params)
{
    ...
    for (;;) {
        ...
        //GY63_Convert(&hgy63, sample.Baro_P, sample.Baro_T, &pressure, &temperature);
        	//printf("Pressure raw: %lu, Temperature raw: %lu |  P=%.2f mbar  T=%.2f C\r\n", sample.Baro_P, sample.Baro_T, pressure, temperature);
        	//printf("Timestamp: %u\n", sample.timestamp);
